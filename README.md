...
<p align="center">一个桌面应用，统一管理 Kimi Code、DeepSeek、MiniMax、OpenCode Go、Command Code、Codex 及自定义供应商的账号、请求和用量。</p>
...
- **多供应商、多账号，一个入口**：将 Kimi Code、DeepSeek、MiniMax、OpenCode Go、Command Code、Codex 和自定义供应商汇入同一本地账号池。
- **智能调度**：优先复用会话绑定，结合并发与剩余额度选择账号，故障时自动切换。
- **模型统一管理**：按调用名称查看模型来源与实际映射，支持账号级模型 ID 映射和逐模型复制 Kimi Code 配置。
...
| 模型列表与 ID 映射     | 按模型、供应商或账号搜索来源；支持精确／通配符映射，逐模型复制 Kimi Code 配置             |
...
| AI 用量小票            | 今天／近 7 天／近 30 天统计，打印与拖拽撕纸、费用／Token 切换、渐变背景和高清 PNG 保存／复制 |
...
| 请求期间阻止休眠       | macOS 实验性功能，支持请求结束后延迟恢复及仅接通电源时启用                              |
| 应用更新               | 自动检查并下载正式版，在更新弹窗中查看新版本更新内容                                    |
...
在概览的「用量统计」中点击「打印小票」，选择今天、近 7 天或近 30 天。打印完成后拖拽撕下小票，或直接预览；可切换费用／Token 主数字、选择背景并保存或复制 PNG。背景选择保存在本机。启用系统“减少动态效果”时会直接进入预览。

小票仅统计 Navo 网关中已记录且已报告用量的请求，生成后固定数据。费用与概览使用同一口径：上游报告优先，否则按当前模型价格估算；缺失价格／用量按 0 计，不同币种分别展示。客户端归属从此版本开始随请求保存，旧记录显示“未知客户端”；会话数仅统计有会话标识的请求并去重。此功能当前仅支持桌面端。

## 支持的供应商

| 供应商       | 账号类型                                      | 同步信息                                       |
| ------------ | --------------------------------------------- | ---------------------------------------------- |
| Kimi Code    | 中国区／国际区 API Key 或本地 OAuth 登录（仅 macOS 导入）      | 模型、5 小时／7 天额度、并发上限               |
...
| Command Code | GOAT 或更高套餐的 API Key                   | 模型、5 小时／周窗口、月剩余与充值／免费额度   |
...
1. 打开「设置 → 账号管理」添加账号：选择内置供应商并填写 API Key / Subscription Key，或填写自定义供应商的 Base URL 与 Key。macOS 上的 Kimi 本地登录态可在添加账号时导入；Codex 认证从「设置 → 实验性功能」导入，详见[账号与模型](#账号与模型)。
...
安装版启动 10 秒后检查 GitHub 正式版，此后每 6 小时检查一次，也可点击右下角「检查更新」。发现新版会提示并后台下载；点击「查看更新」可在弹窗内阅读新版本更新内容，未提供说明时可跳转发布页。下载完成后点击「重启并安装」即可自动安装并重新打开，账号和设置保留。安装前会确认重启，因为这会中断正在处理的网关请求。开发模式不检查更新，预发布版不推送给用户。
...
安装版可在状态栏／托盘菜单中直接勾选或取消「开机自启 Navo」，也可在「设置 → 网关设置 → 应用启动」中切换；登录系统后自动打开应用。若还希望网关自动运行，另行开启「打开应用时自动启动网关」并保存网关设置。macOS 可能要求在系统登录项设置中允许 Navo；Linux 使用当前 AppImage 路径，请将文件放在固定位置。开发模式不修改系统登录项。
...
在同一屏查看账号可调度状态、额度窗口、模型表现和 Token 活动。桌面概览刷新时保留已有数据，避免加载占位与数值闪烁；Token、请求数和费用等指标使用数字滚动过渡。
...
按请求查看账号、模型、协议转换、思考强度、状态、首字耗时、总耗时和费用，便于定位失败和比较性能。模型名称带品牌图标，发生映射时同时展示客户端模型 ID 和实际调用模型；「耗时」列分行展示首字与总耗时，并用颜色区分延迟区间，悬停可查看毫秒值。协议转换列分别显示入站与实际上游路径，未发往上游的请求显示「未转发」；列表不再展示 Request ID。记录保留最近 90 天，每页 10 条，模型测试带有独立标记。
...
<details>
<summary>查看模型列表与复制 Kimi Code 配置</summary>

点击顶栏「可用模型」，按调用名称查看汇总模型及其供应商、账号来源，支持搜索模型、供应商、账号和实际调用模型。每个来源会标明「同名直连」或「映射至」的目标；已停用或未配置凭据的账号也会列出并标注状态，因此列表不代表所有来源当前都可调度。

点击模型旁的复制按钮，可生成该模型的 Kimi Code TOML 配置片段，包含模型 ID、上下文长度、展示名，以及已知的工具调用、思考、图片输入能力和思考档位。导出仅使用已启用、具备凭据且同步成功的来源；多个来源取最小上下文长度和共同支持的能力。缺少上下文信息时会提示先在模型定价中关联模型目录，不会猜测数值。

片段使用 `provider = "Navo"`，只包含模型配置，不含供应商地址或密钥。合并到 Kimi 配置时，需要已有同名的 `[providers.Navo]`，或将片段中的供应商名称改为你实际使用的配置名称。模型列表中的调用名称和精确映射别名均可按此方式导出。
...
<details>
<summary>手动补充模型与模型 ID 映射</summary>
...
「编辑账号 → 模型 ID 映射」可为每个账号配置请求 ID 到上游 ID 的映射，例如 `claude-sonnet-4-6 → kimi-for-coding`。匹配参考 [sub2api](https://github.com/Wei-Shaw/sub2api/blob/main/backend/internal/service/account.go)：精确匹配优先，再按末尾 `*` 匹配最长前缀（如 `claude-*`）；`*` 可作为兜底规则。只映射一次，未命中时使用原 ID。目标必须在该账号的可用模型中启用；上游未列出的目标可先手动添加。映射保存后立即生效，刷新及重启后保留，清空映射即可恢复原行为。

调度会按每个账号的映射结果检查模型是否可用，故障切换时重新映射；上游协议和费用估算使用实际目标模型。`/v1/models` 与 `/api.json` 会列出可调用的精确别名，通配符规则本身不作为模型 ID 列出。请求记录保留客户端 ID 并显示实际上游 ID；上游响应中的模型名保持实际模型。未命中映射的原有模型仍可直接调用，如需禁用模型，请在可用模型列表中删除。

模型性能统计按实际调用的上游模型合并，同一账号中映射别名与直接调用同一模型的请求共同统计；旧记录缺少上游模型 ID 时使用原模型名。
...
本地 Kimi 登录态导入目前仅支持 macOS，其他平台请使用 API Key。先在本机 Kimi Code 中登录，再打开「设置 → 账号管理」，点击「添加账号」，在供应商中选择「Kimi Code · 本地登录态」，选择登录账号所在区域并导入。导入后可直接使用网关，同一账号重复导入会更新认证，保留名称、启停状态、分组和手动模型。
...
以下示例使用默认端口。请替换 `<GATEWAY_KEY>` 为应用中复制的网关密钥，模型 ID 必须存在于已启用账号的可用模型列表中，或通过映射指向该列表中的模型。上游 API Key 填在应用中。
...
可从顶栏「可用模型」逐模型复制配置片段，使用方法见[账号与模型](#账号与模型)。也可按下面的示例手动配置，将内容合并到 `~/.kimi/config.toml`，保留其他设置；`default_model` 应位于文件顶层：
...
仪表盘支持按供应商筛选，展示 Codex、MiniMax、Command Code 等账号的可用额度窗口、重置时间和用量。桌面额度概览与远程仪表盘只展示已有额度或余额数据的账号；没有这些数据的账号（如自定义供应商）不显示额度卡片，仍可在账号管理中配置和调用。剩余额度为 0 的账号仍会显示。
...
此功能仅支持 **macOS**。在「设置 → 实验性功能 → 请求期间阻止休眠」开启，默认关闭。最后一个网关请求结束后默认延迟 1 分钟恢复，可选择 1 分钟、5 分钟、15 分钟、30 分钟或 1 小时；期间新请求会取消恢复倒计时。可开启「仅连接电源适配器时启用」（默认关闭）：切换到电池供电时立即释放，接回电源且仍有请求时恢复阻止休眠。

macOS 使用系统级 `pmset disablesleep`，生效时苹果菜单的「睡眠」不可用，包含合盖睡眠；屏幕仍按原设置休眠。每次启动应用后需完成一次 macOS 管理员授权，同次运行内不需为每个请求重复授权。Windows / Linux 暂不支持此功能。

关闭开关、退出应用或请求结束后的延迟到期时恢复。临时辅助进程独立监控应用和心跳，应用崩溃或失去响应后约 10 秒内恢复；不安装永久后台服务，也不修改 sudoers。若系统原本已被其他工具禁用睡眠，Navo 保留该状态。授权或恢复失败会在设置页显示，必要时可在终端执行 `sudo pmset -a disablesleep 0` 手动恢复系统睡眠。
...
- 请求摘要保存在同目录的 `gateway.json.requests.sqlite`，自动清理超过 90 天的记录，历史统计受该保留范围限制；主题保存在 `settings.json`。macOS 安装版默认目录为 `~/Library/Application Support/Navo/`，源码运行使用独立的 `~/Library/Application Support/Navo Dev/`。
...
源码运行（`npm run dev` 和 `npm start`）使用独立的 `Navo Dev` 数据目录，安装版继续使用 `Navo`，避免新版开发数据导致旧安装版无法启动。源码模式首次启动需要单独配置账号；自动化测试仍通过 `NAVO_TEST_USER_DATA` 指定临时目录。
