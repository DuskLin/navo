...
在「设置 → 实验性功能 → 请求期间阻止休眠」开启，默认关闭。最后一个网关请求结束后默认延迟 1 分钟恢复，可选择 1 分钟、5 分钟、15 分钟、30 分钟或 1 小时；期间新请求会取消恢复倒计时。可开启「仅连接电源适配器时启用」（默认关闭）：切换到电池供电时立即释放，接回电源且仍有请求时恢复阻止休眠。

macOS 使用系统级 `pmset disablesleep`，生效时苹果菜单的「睡眠」不可用，包含合盖睡眠；屏幕仍按原设置休眠。每次启动应用后需完成一次 macOS 管理员授权，同次运行内不需为每个请求重复授权。其他平台使用 Electron 的自动休眠阻止接口。
